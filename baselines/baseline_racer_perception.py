...
class BaselineRacerPerception(BaselineRacer):
    def __init__(self, drone_name = "drone_1", plot_transform=True, viz_traj=True):
        ...
        self.last_good_waypoint = airsim.Vector3r(0,0,0)
        self.kernel = np.ones((3,3),np.uint8)
        self.dst_pts = np.float32([[[200, 132],[412, 132],[412, 346], [200, 346]]])
    ...
    def find_gate_corners(self, hull_comp):
    ...
    def find_aspect_ratio(self, hull_comp):
        aspect_ratio_mtx = np.empty((1,4))
        aspect_ratio_mtx[0,0] = abs(np.float32(hull_comp[1][0][1] - hull_comp[2][0][1])/np.float32(hull_comp[0][0][0] - hull_comp[1][0][0]+self.eps))
        aspect_ratio_mtx[0,1] = abs(np.float32(hull_comp[0][0][1] - hull_comp[3][0][1])/np.float32(hull_comp[2][0][0] - hull_comp[3][0][0]+self.eps))
        aspect_ratio_mtx[0,2] = abs(np.float32(hull_comp[0][0][1] - hull_comp[3][0][1])/np.float32(hull_comp[0][0][0] - hull_comp[1][0][0]+self.eps))
        aspect_ratio_mtx[0,3] = abs(np.float32(hull_comp[1][0][1] - hull_comp[2][0][1])/np.float32(hull_comp[2][0][0] - hull_comp[3][0][0]+self.eps))
    ...
    def run(self):
        ...
        self.airsim_client.moveOnSplineAsync([self.gate_poses_ground_truth[0].position], vel_max = 2.0, acc_max = 5.0, viz_traj=self.viz_traj, vehicle_name=self.drone_name).join()
        ...
        while self.airsim_client.isApiControlEnabled(vehicle_name=self.drone_name):
            ...
            dilated_comp = cv2.dilate(mask,self.kernel, iterations=8)
            eroded_comp = cv2.erode(dilated_comp,self.kernel, iterations=8)
            ...
            for c in contours_comp:
                ...
                if cv2.contourArea(c) > largest_area:
                    ...
                    approx_comp = cv2.approxPolyDP(c,self.epsilon_comp,True)
                    ...
                    if len(hull_comp)>4:
                        hull_comp = self.find_gate_corners(hull_comp)
                        hull_comp = hull_comp.astype(int).reshape(4,1,2)
                    if len(hull_comp)>=4:
                        aspect_ratio = self.find_aspect_ratio(hull_comp)
                        if aspect_ratio <= 1.2:
            ...
            if largest_area < 3500 or len(hull_comp_best)!=4:
                ...
                if self.no_gate_count > 25:
                    self.airsim_client.moveOnSplineAsync([self.last_good_waypoint + airsim.Vector3r(np.random.uniform(low=-0.25,high=0.25,size=1)[0],
                                                np.random.uniform(low=-0.25,high=0.25,size=1)[0],
                                                np.random.uniform(low=-0.25,high=0.25,size=1)[0])],
                                            vel_max = 10.0, acc_max = 5.0, viz_traj=self.viz_traj, vehicle_name=self.drone_name).join() # move to last good waypoint
                    self.no_gate_count = 0
                # Rotate counterclockwise until gate has been found
                else:
                    self.airsim_client.moveByYawRateAsync(yaw_rate=-15.0, duration=0.5, vehicle_name=self.drone_name).join() # rotate counter clockwise to look for next gate
            else:
                # gate corner points
                ...
                M, mask = cv2.findHomography(src_pts, self.dst_pts, cv2.RANSAC, 5.0)
                ...
                center_pixel_comp_norm = (int(center_pixel_comp[0]/(center_pixel_comp[2]+self.eps)), int(center_pixel_comp[1]/(center_pixel_comp[2]+self.eps)))
                ...
                norm_dist_error = abs(np.linalg.norm(waypoint_glob_2) - np.linalg.norm(self.waypoint_ave_2))
                ...
                self.airsim_client.moveOnSplineAsync([waypoint_1,waypoint_2,waypoint_3], vel_max = 5.0, acc_max = 2.0, add_curr_odom_position_constraint=True, add_curr_odom_velocity_constraint=False, viz_traj=self.viz_traj, vehicle_name=self.drone_name).join()
                self.last_good_waypoint = copy.deepcopy(waypoint_3)
